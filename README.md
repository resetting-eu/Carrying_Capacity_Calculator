...
A paper describing the features of this calculator from an end-user perspective and the algorithm used in this tool is available [HERE](https://dx.doi.org//??????????).
...
```
Sampaio de Almeida, D., Simões, R., Brito e Abreu, F., & Lopes, A. & Boavida-Portugal, Inês (2024, May). Guide to the carrying capacity calculator for pedestrians on OpenStreetMap. https://dx.doi.org/?????????????
```
...
@techreport{Almeida2024,
    ...
    title = {Guide to the carrying capacity calculator for pedestrians on {OpenStreetMap}},
    institution = {arXiv},
    year = {2024},
    month = {May},
    doi = {?????????????}
